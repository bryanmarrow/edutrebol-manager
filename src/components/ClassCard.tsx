...
interface ClassCardProps {
    ...
    onEdit?: () => void;
    onDelete?: () => void;
    /** When provided, tapping the card calls this instead of navigating directly */
    onSelect?: () => void;
}

export function ClassCard({ id, name, group, startTime, endTime, status, studentCount, onEdit, onDelete, onSelect }: ClassCardProps) {
    ...
    return (
        <div className={cn(
            ...
            {/* Main area — confirms before navigating to attendance */}
            {onSelect ? (
                <button type="button" onClick={onSelect} className="block w-full text-left p-4 transition-transform active:scale-[0.98]">
                    {cardBody}
                </button>
            ) : (
